...
The outcome of every notification is recorded on the enquiry itself and shown quietly in the admin detail view, so a mail failure is visible in the panel instead of disappearing for months.
...
- `leads`: add `read_at timestamptz`, `archived_at timestamptz`, `project_type text`, `timeline text`, `notified_at timestamptz`, `notify_error text`; index on `(archived_at, read_at, created_at desc)`.
...
**Trigger → function**
- `AFTER INSERT ON public.leads` trigger runs a security-definer function that calls the edge function `notify-inquiry` asynchronously with `net.http_post` (pg_net), passing the lead id. A dispatch problem can never block the submission, but it is never silent either — the outcome is recorded on the lead row rather than swallowed.
- New edge function `supabase/functions/notify-inquiry/index.ts`: validates the payload with Zod, loads the lead and the recipient list with the service role, renders the existing `inquiry-notification` template (extended with project type, timeline and an "Open in admin" link), and sends via `sendLovableEmail`. On success it writes `notified_at`; on failure it writes the reason to `notify_error` (and clears `notified_at`). A lead left with neither is itself a signal that the notification never ran. CORS headers imported from `npm:@supabase/supabase-js@2/cors`.
...
- `src/pages/admin/AdminInquiries.tsx` — list + detail (Sheet on desktop/mobile), matching the existing admin patterns (search bar, filter dropdown, table/list styling used by AdminProjects). The detail view carries a quiet notification-status line in muted text: "Notification sent <date>", the recorded error if it failed, or "Notification not sent" when neither is recorded.
